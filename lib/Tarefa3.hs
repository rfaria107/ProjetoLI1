...
moveInimigos [] _ = []
moveInimigos ((f1@(Personagem (vx,vy) MacacoMalvado (x,y) dir (c,l) esc ress vidas pon (n,z)):fs)) m1 = f1 : moveInimigos fs m1
moveInimigos ((f1@(Personagem (vx,vy) Fantasma (x,y) dir (c,l) esc ress vidas pon (n,z)):fs)) m1
                          |vx>0 && (x<17) = (Personagem (vx,vy) Fantasma (x+0.02,y) dir (c,l) esc ress vidas pon (n,z)) : moveInimigos fs m1
                          |vx>0 && (x>17) = (Personagem (vx,vy) Fantasma (x-0.02,y) dir (c,l) esc ress vidas pon (n,z)) : moveInimigos fs m1
                          |vx<0 && (x>17) = (Personagem (vx,vy) Fantasma (x-0.02,y) dir (c,l) esc ress vidas pon (n,z)) : moveInimigos fs m1
                          |vx<0 && (x<17) = (Personagem (vx,vy) Fantasma (x+0.02,y) dir (c,l) esc ress vidas pon (n,z)) : moveInimigos fs m1
                          |otherwise = f1 : moveInimigos fs m1