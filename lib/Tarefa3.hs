...
movimenta semente tempo jogo1@(Jogo mapa1 inimigos1@(i:is) colecionaveis jogador1) =  jogo1 {
      jogador = apanharMoeda (jogador jogo1) jogo1,
      inimigos = map (aplicarDanoMartelo jogador1) inimigos1
}
...
aplicarDanoMartelo p1@(Personagem _ Jogador _ _ _ _ _ _ _ (True,_)) p2@(Personagem velocidade Fantasma pos dir (c,l) esc ress vidas pon (n,z))        |colisaoHitbox (hitboxDano p1) (defineHitbox p2) = (Personagem velocidade Fantasma pos dir (c,l) esc ress (vidas-1) pon (n,z)) 
                                                                                                                                                      |otherwise = p2
...
--desaparecerColecionavel :: Personagem -> Jogo -> Picture
--desaparecerColecionavel p1@(Personagem vel Jogador (x,y) dir (c,l) esc ress vidas pon (n,z)) j1@(Jogo _ _ col _)  |
--                                                                                                                  |otherwise = Blank